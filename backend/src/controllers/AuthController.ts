...
export class AuthController {
  ...
  async registerEmail(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const user = await authService.registerWithEmail(email, password, name || email.split('@')[0]);
  ...
  async loginEmail(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      delete result.user.github_token;
      delete result.user.figma_token;
      ...
      if (error.message === 'This account uses social login') {
        return res.status(401).json({
          error: 'This account uses social login. Please login with GitHub.',
        });
      }
      next(error);
    }
  ...
  async me(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      delete (user as any).github_token;
      delete (user as any).figma_token;
